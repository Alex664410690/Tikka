...
module Tikka (parseAndEvaluateProject, Flags (..), Parser, lexeme, isFunction, addToLocalDecl, getExecutablePath') where
...
-- Gets the path of the executable (Tikka/dist/ when built, Tikka/.stack-work/ when run from stack)
getExecutablePath' :: IO FilePath
getExecutablePath' = do
  exePath <- getExecutablePath
  if ".stack-work" `isInfixOf` exePath then pure $ takeDirectory $ takeDirectory $ takeDirectory exePath else pure exePath
...
parseAndEvaluateProject :: Flags -> String -> String -> FilePath -> IO (Either String (String, [String], String))
parseAndEvaluateProject flags file fp func = do 
  exePath <- getExecutablePath'
  let dirPath p = takeDirectory (takeDirectory exePath) </> p

  case parse (getImports fp dirPath) "" (pack file) of
...
getImports :: FilePath -> (FilePath -> FilePath) -> Parser (String, [String])
getImports fp dirPath = do
  files <- many getImport
  file <- takeRest
  pure (unpack file, map (fp </>) files ++ [dirPath "imports/command-line", dirPath "imports/local-decl", dirPath "imports/prelude"])
...
addToLocalDecl s = do
  ...
  file <- S.readFile $ dirPath "imports/local-decl"
  let toWrite = addDecl s (Prelude.lines file)
  writeFile (dirPath "imports/local-decl") (Prelude.unlines toWrite)
