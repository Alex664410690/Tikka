-- SOURCE: Monomer.Widgets.Singles.TextArea
...
{-# OPTIONS_GHC -w #-} -- disables all warnings from poorly written source code!

module TextAreaScroll (
  TextAreaScrollMessage (..),
  -- * Configuration
  TextAreaCfg,
  -- * Constructors
  textArea,
...
data TextAreaScrollMessage = Insert Text
  deriving (Eq, Show)
...
textArea field moveEvent = textArea_ field def moveEvent
...
textArea_
  ...
  -> (ScrollMessage -> e)
  -> WidgetNode s e     -- ^ The created text area.
textArea_ field configs moveEvent = textAreaD_ wdata configs moveEvent where
...
textAreaV
  ...
  -> (ScrollMessage -> e)
  -> WidgetNode s e  -- ^ The created text area.
textAreaV value handler moveEvent = textAreaV_ value handler def moveEvent
...
textAreaV_
  ...
  -> (ScrollMessage -> e)
  -> WidgetNode s e     -- ^ The created text area.
textAreaV_ value handler configs moveEvent = textAreaD_ wdata newConfig moveEvent where
...
textAreaD_
  ...
  -> (ScrollMessage -> e)
  -> WidgetNode s e     -- ^ The created text area.
textAreaD_ wdata configs moveEvent = node where
  config = mconcat configs
  widget = makeTextArea wdata config def moveEvent
...
makeTextArea
  ...
  -> (ScrollMessage -> e)
  -> Widget s e
makeTextArea !wdata !config !state moveEvent = widget where
  ...
  init wenv node = resultNode newNode where
    ...
    newNode = node
      & L.widget .~ makeTextArea wdata config newState moveEvent
  ...
  handleMessage wenv node target message = result where
    handleTAScrollMessage (Insert text) = Just $ insertText wenv node text
    result = cast message >>= handleTAScrollMessage
  ...
  generateScrollReq wenv node newState = scrollReq where
    ...
    scrollReq
      | rectInRect caretRect (wenv ^. L.viewport) || isNothing scWid = [RaiseEvent (moveEvent scrollMsg)]
      | otherwise = [SendMessage (fromJust scWid) scrollMsg, RaiseEvent (moveEvent scrollMsg)]
